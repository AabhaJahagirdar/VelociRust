...
$proxy = (Join-Path $directory 'velocirust.exe')

$VelocirustProcesses = (Get-Process -Name 'velocirust' -EA SilentlyContinue).Count
if ($VelocirustProcesses -ne 0) {
...
$url = "https://github.com/velocirust/velocirust/releases/download/${version}/velocirust-proxy-windows-${arch}.gz"
$gzip = Join-Path "${env:TMP}" "velocirust-proxy-windows-${arch}.gz"
...
Write-Host 'velocirust-proxy installed'