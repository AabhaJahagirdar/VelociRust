...
It is easy to build Velocirust from source on a GNU/Linux distribution. Cargo handles the build process, all you need to do, is ensure the correct dependencies are installed.
...
```sh
git clone https://github.com/velocirust/velocirust.git ~/velocirust
```
...
```sh
cargo install --path . --bin velocirust --profile release-lto --locked
```

> If you use a different distribution, and are having trouble finding appropriate dependencies, let us know in an issue!

Once Velocirust is compiled, the executable will be available in `$HOME/.cargo/bin/velocirust` and should be available in `PATH` automatically.
...
Additionally to building multiple OS versions at the same time, Docker-based builds will also try to cross-compile Velocirust for other architectures.
