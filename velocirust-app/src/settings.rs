...
use velocirust_core::{buffer::rope_text::RopeText, mode::Mode};
...
use crate::{
    ...
    config::{
        DropdownInfo, VelocirustConfig, color::VelocirustColor, core::CoreConfig,
        editor::EditorConfig, icon::VelocirustIcons, terminal::TerminalConfig,
        ui::UIConfig,
    },
...
impl KeyPressFocus for SettingsData {
    fn get_mode(&self) -> velocirust_core::mode::Mode {
        Mode::Insert
    }
    ...
    fn run_command(
        &self,
        _command: &crate::command::VelocirustCommand,
        _count: Option<usize>,
        _mods: Modifiers,
        ...
        .style(move |s| {
            ...
            s.padding_horiz(20.0)
                ...
                .apply_if(kind == current_kind.get(), |s| {
                    s.background(config.color(VelocirustColor::PANEL_CURRENT_BACKGROUND))
                })
                .hover(|s| {
                    ...
                    let token =
                        exec_after(Duration::from_millis(500), move |token| {
                            ...
                            if let Some(value) = value {
                                VelocirustConfig::update_file(&kind, &field, value);
                            }
                        });
                    timer.set(token);
                                ...
                                if let Some(timer) = timer.try_get_untracked() {
                                    if timer == token {
                                        ...
                                        if default != Some(&value) {
                                            ...
                                            VelocirustConfig::reset_setting(
                                                &format!("color-theme.{kind}"),
                                                &field,
                                            );
                                        }
                        ...
                        text("Reset")
                            ...
                            .style(move |s| {
                                ...
                                s.margin_left(10)
                                    ...
                                    .border_radius(6)
                                    .border_color(
                                        config.color(VelocirustColor::LAPCE_BORDER),
                                    )
...
fn dropdown_view(
    ...
    stack((
        ...
        container(
            svg(move || {
                if expanded.get() {
                    config.get().ui_svg(VelocirustIcons::CLOSE)
                } else {
                    config.get().ui_svg(VelocirustIcons::DROPDOWN_ARROW)
                }
            })
            .style(move |s| {
                let config = config.get();
