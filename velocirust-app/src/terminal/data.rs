...
impl ExpandedRunDebug {
    pub fn expand(
        ...
        let mut program = if program == "${velocirust}" {
            std::env::current_exe()
                .map_err(|e| {
                    anyhow!(
                        "Failed to get current exe for ${{velocirust}} run and debug: {e}"
                    )
                })?
                .to_str()
                .ok_or_else(|| anyhow!("Failed to convert ${{velocirust}} path to str"))?
                .to_string()
        } else {
            program
        };
