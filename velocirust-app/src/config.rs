...
use velocirust_core::directory::Directory;
use velocirust_proxy::plugin::wasi::find_all_volts;
use velocirust_rpc::plugin::VoltID;
...
static DEFAULT_CONFIG: Lazy<config::Config> = Lazy::new(VelocirustConfig::default_config);
static DEFAULT_LAPCE_CONFIG: Lazy<VelocirustConfig> =
    Lazy::new(VelocirustConfig::default_velocirust_config);
...
static DEFAULT_DARK_THEME_COLOR_CONFIG: Lazy<ColorThemeConfig> = Lazy::new(|| {
    let (_, theme) =
        VelocirustConfig::load_color_theme_from_str(DEFAULT_DARK_THEME).unwrap();
...
impl VelocirustConfig {
    pub fn load(
        workspace: &VelocirustWorkspace,
        disabled_volts: &[VoltID],
        extra_plugin_paths: &[PathBuf],
    ) -> Self {
        let config = Self::merge_config(workspace, None, None);
        let mut velocirust_config: VelocirustConfig = match config.try_deserialize() {
            Ok(config) => config,
            Err(error) => {
        ...
        velocirust_config.available_color_themes =
            Self::load_color_themes(disabled_volts, extra_plugin_paths);
        velocirust_config.available_icon_themes =
            Self::load_icon_themes(disabled_volts, extra_plugin_paths);
        velocirust_config.resolve_theme(workspace);

        velocirust_config.color_theme_list = velocirust_config
            .available_color_themes
            .values()
            .map(|(name, _)| name.clone())
            .sorted()
            .collect();
        velocirust_config.color_theme_list.sort();

        velocirust_config.icon_theme_list = velocirust_config
            .available_icon_themes
            .values()
            .map(|(name, _, _)| name.clone())
            .sorted()
            .collect();
        velocirust_config.icon_theme_list.sort();

        velocirust_config.wrap_style_list = im::vector![
        ...
        velocirust_config.terminal.get_indexed_colors();
        ...
        match workspace.kind {
            VelocirustWorkspaceType::Local => {
                if let Some(path) = workspace.path.as_ref() {
                    let path = path.join("./.velocirust/settings.toml");
                    config = config::Config::builder()
                        .add_source(config.clone())
                        .add_source(
            ...
            VelocirustWorkspaceType::RemoteSSH(_) => {}
            #[cfg(windows)]
            VelocirustWorkspaceType::RemoteWSL(_) => {}
        }
    ...
    fn default_velocirust_config() -> VelocirustConfig {
        let mut default_velocirust_config: VelocirustConfig =
        ...
        default_velocirust_config.color_theme = DEFAULT_DARK_THEME_COLOR_CONFIG.clone();
        default_velocirust_config.icon_theme = DEFAULT_ICON_THEME_ICON_CONFIG.clone();
        default_velocirust_config.resolve_colors(None);
        default_velocirust_config
    }

    fn resolve_theme(&mut self, workspace: &VelocirustWorkspace) {
        let default_velocirust_config = DEFAULT_LAPCE_CONFIG.clone();
        ...
        .try_deserialize::<VelocirustConfig>()
        {
            self.core = new.core;
            self.ui = new.ui;
        ...
        self.resolve_colors(Some(&default_velocirust_config));
        self.update_id();
    }
    ...
    pub fn set_color_theme(&mut self, workspace: &VelocirustWorkspace, theme: &str) {
    ...
    /// Note that this does not save the config.
    pub fn set_icon_theme(&mut self, workspace: &VelocirustWorkspace, theme: &str) {
        self.core.icon_theme = theme.to_string();
        self.resolve_theme(workspace);
    }

    pub fn set_modal(&mut self, _workspace: &VelocirustWorkspace, modal: bool) {
    ...
    fn resolve_colors(&mut self, default_config: Option<&VelocirustConfig>) {
        ...
        let fg = self.color(VelocirustColor::EDITOR_FOREGROUND).to_rgba8();
        let bg = self.color(VelocirustColor::EDITOR_BACKGROUND).to_rgba8();
    ...
    pub fn files_svg(&self, paths: &[&Path]) -> (String, Option<Color>) {
        ...
        if let Some(svg) = svg {
            let color = if self.icon_theme.use_editor_color.unwrap_or(false) {
                Some(self.color(VelocirustColor::LAPCE_ICON_ACTIVE))
            } else {
                None
            };
            (svg, color)
            ...
            (
                self.ui_svg(VelocirustIcons::FILE),
                Some(self.color(VelocirustColor::LAPCE_ICON_ACTIVE)),
            )
        }
    }
    ...
    pub fn symbol_svg(&self, kind: &SymbolKind) -> Option<String> {
        let kind_str = match *kind {
            SymbolKind::ARRAY => VelocirustIcons::SYMBOL_KIND_ARRAY,
            SymbolKind::BOOLEAN => VelocirustIcons::SYMBOL_KIND_BOOLEAN,
            SymbolKind::CLASS => VelocirustIcons::SYMBOL_KIND_CLASS,
            SymbolKind::CONSTANT => VelocirustIcons::SYMBOL_KIND_CONSTANT,
            SymbolKind::ENUM_MEMBER => VelocirustIcons::SYMBOL_KIND_ENUM_MEMBER,
            SymbolKind::ENUM => VelocirustIcons::SYMBOL_KIND_ENUM,
            SymbolKind::EVENT => VelocirustIcons::SYMBOL_KIND_EVENT,
            SymbolKind::FIELD => VelocirustIcons::SYMBOL_KIND_FIELD,
            SymbolKind::FILE => VelocirustIcons::SYMBOL_KIND_FILE,
            SymbolKind::INTERFACE => VelocirustIcons::SYMBOL_KIND_INTERFACE,
            SymbolKind::KEY => VelocirustIcons::SYMBOL_KIND_KEY,
            SymbolKind::FUNCTION => VelocirustIcons::SYMBOL_KIND_FUNCTION,
            SymbolKind::METHOD => VelocirustIcons::SYMBOL_KIND_METHOD,
            SymbolKind::OBJECT => VelocirustIcons::SYMBOL_KIND_OBJECT,
            SymbolKind::NAMESPACE => VelocirustIcons::SYMBOL_KIND_NAMESPACE,
            SymbolKind::NUMBER => VelocirustIcons::SYMBOL_KIND_NUMBER,
            SymbolKind::OPERATOR => VelocirustIcons::SYMBOL_KIND_OPERATOR,
            SymbolKind::TYPE_PARAMETER => VelocirustIcons::SYMBOL_KIND_TYPE_PARAMETER,
            SymbolKind::PROPERTY => VelocirustIcons::SYMBOL_KIND_PROPERTY,
            SymbolKind::STRING => VelocirustIcons::SYMBOL_KIND_STRING,
            SymbolKind::STRUCT => VelocirustIcons::SYMBOL_KIND_STRUCT,
            SymbolKind::VARIABLE => VelocirustIcons::SYMBOL_KIND_VARIABLE,
            _ => return None,
        };
    ...
    fn terminal_get_named_color(
        ...
        let (color, alpha) = match color {
            alacritty_terminal::vte::ansi::NamedColor::Cursor => {
                (VelocirustColor::TERMINAL_CURSOR, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Foreground => {
                (VelocirustColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Background => {
                (VelocirustColor::TERMINAL_BACKGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Blue => {
                (VelocirustColor::TERMINAL_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Green => {
                (VelocirustColor::TERMINAL_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Yellow => {
                (VelocirustColor::TERMINAL_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Red => {
                (VelocirustColor::TERMINAL_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::White => {
                (VelocirustColor::TERMINAL_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Black => {
                (VelocirustColor::TERMINAL_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Cyan => {
                (VelocirustColor::TERMINAL_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Magenta => {
                (VelocirustColor::TERMINAL_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlue => {
                (VelocirustColor::TERMINAL_BRIGHT_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightGreen => {
                (VelocirustColor::TERMINAL_BRIGHT_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightYellow => {
                (VelocirustColor::TERMINAL_BRIGHT_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightRed => {
                (VelocirustColor::TERMINAL_BRIGHT_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightWhite => {
                (VelocirustColor::TERMINAL_BRIGHT_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlack => {
                (VelocirustColor::TERMINAL_BRIGHT_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightCyan => {
                (VelocirustColor::TERMINAL_BRIGHT_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightMagenta => {
                (VelocirustColor::TERMINAL_BRIGHT_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightForeground => {
                (VelocirustColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlack => {
                (VelocirustColor::TERMINAL_BLACK, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimRed => {
                (VelocirustColor::TERMINAL_RED, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimGreen => {
                (VelocirustColor::TERMINAL_GREEN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimYellow => {
                (VelocirustColor::TERMINAL_YELLOW, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlue => {
                (VelocirustColor::TERMINAL_BLUE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimMagenta => {
                (VelocirustColor::TERMINAL_MAGENTA, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimCyan => {
                (VelocirustColor::TERMINAL_CYAN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimWhite => {
                (VelocirustColor::TERMINAL_WHITE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimForeground => {
                (VelocirustColor::TERMINAL_FOREGROUND, 0.66)
            }
        };
