...
mod tests {
    ...
    use crate::{config::VelocirustConfig, workspace::VelocirustWorkspace};

    #[test]
    fn test_resolve() {
        // Mimicking load
        let workspace = VelocirustWorkspace::default();

        let config = VelocirustConfig::merge_config(&workspace, None, None);
        let mut velocirust_config: VelocirustConfig = config.try_deserialize().unwrap();
...
"##;
        ...
        velocirust_config.available_color_themes =
            [("test".to_string(), ("test".to_string(), test_theme_cfg))]
                .into_iter()
                .collect();
        // velocirust_config.available_icon_themes = Some(vec![]);
        velocirust_config.core.color_theme = "test".to_string();
