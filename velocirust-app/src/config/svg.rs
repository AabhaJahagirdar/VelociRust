...
const LAPCE_ICONS_DIR: Dir = include_dir!("$CARGO_MANIFEST_DIR/../icons/velocirust");
...
impl SvgStore {
    fn new() -> Self {
        ...
        svgs.insert("velocirust_logo".to_string(), LOGO.to_string());
    ...
    pub fn logo_svg(&self) -> String {
        self.svgs.get("velocirust_logo").unwrap().clone()
    }

    pub fn get_default_svg(&mut self, name: &str) -> String {
        if !self.svgs.contains_key(name) {
            let file = if name == "velocirust_remote.svg" || name == "velocirust_logo.svg" {
