...
use velocirust_rpc::{buffer::BufferId, plugin::PluginId, proxy::ProxyResponse};
...
impl EditorData {
    ...
    fn run_edit_command(&self, cmd: &EditCommand) -> CommandExecuted {
        ...
        let yank_data =
            if let velocirust_core::cursor::CursorMode::Visual { .. } = &cursor.mode {
    ...
    fn run_move_command(
        &self,
        movement: &velocirust_core::movement::Movement,
        count: Option<usize>,
        mods: Modifiers,
    ) -> CommandExecuted {
            ...
            FocusCommand::JumpToNextSnippetPlaceholder => {
                self.snippet.update(|snippet| {
                    if let Some(snippet_mut) = snippet.as_mut() {
                        ...
                        {
                            let mut selection =
                                velocirust_core::selection::Selection::new();
                            let region = velocirust_core::selection::SelRegion::new(
                                *start, *end, None,
                            );
    ...
    fn inline_find(&self, direction: InlineFindDirection, c: &str) {
        ...
        if let Some(new_index) = match direction {
            ...
            self.run_move_command(
                &velocirust_core::movement::Movement::Offset(
    ...
    fn on_screen_find(&self, pattern: &str) -> Vec<SelRegion> {
        ...
        if let Some((_, offset, _)) = items.first().copied() {
            self.run_move_command(
                &velocirust_core::movement::Movement::Offset(offset),
                None,
                Modifiers::empty(),
            );
        }
    ...
    fn apply_completion_item(&self, item: &CompletionItem) -> anyhow::Result<()> {
        ...
        let additional_edit: Vec<_> = item
            ...
            .collect::<Vec<(velocirust_core::selection::Selection, &str)>>();
    ...
    pub fn word_at_cursor(&self) -> String {
        ...
        let region = self.cursor().with_untracked(|c| match &c.mode {
            velocirust_core::cursor::CursorMode::Normal(offset) => {
                velocirust_core::selection::SelRegion::caret(*offset)
            }
            velocirust_core::cursor::CursorMode::Visual {
                start,
                end,
                mode: _,
            } => velocirust_core::selection::SelRegion::new(
                *start.min(end),
                doc.buffer.with_untracked(|buffer| {
            ...
            velocirust_core::cursor::CursorMode::Insert(selection) => {
                        ...
                        && pointer_event.modifiers.control())
                {
                    ...
                    match rs {
                        ...
                        | FindHintRs::NoMatchContinue { .. } => {
                            self.common.velocirust_command.send(VelocirustCommand {
