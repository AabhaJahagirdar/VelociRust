...
use velocirust_rpc::{plugin::PluginId, proxy::ProxyRpcHandler};
use lsp_types::{
...
use crate::{config::VelocirustConfig, editor::EditorData, snippet::Snippet};
...
impl CompletionData {
    pub fn new(cx: Scope, config: ReadSignal<Arc<VelocirustConfig>>) -> Self {
...
fn completion_lens_text(
    ...
    // Get only the first line of text, because Velocirust does not currently support
