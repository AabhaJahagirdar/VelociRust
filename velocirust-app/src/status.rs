...
use velocirust_core::mode::{Mode, VisualMode};
use lsp_types::{DiagnosticSeverity, ProgressToken};

use crate::{
    app::clickable_icon,
    command::VelocirustWorkbenchCommand,
...
pub fn status(
    ...
    _config: ReadSignal<Arc<VelocirustConfig>>,
) -> impl View {
        ...
        stack((
            ...
            .style(move |s| {
                ...
                let (bg, fg) = match mode.get() {
                    Mode::Normal => (
                        VelocirustColor::STATUS_MODAL_NORMAL_BACKGROUND,
                        VelocirustColor::STATUS_MODAL_NORMAL_FOREGROUND,
                    ),
                    Mode::Insert => (
                        VelocirustColor::STATUS_MODAL_INSERT_BACKGROUND,
                        VelocirustColor::STATUS_MODAL_INSERT_FOREGROUND,
                    ),
                    Mode::Visual(_) => (
                        VelocirustColor::STATUS_MODAL_VISUAL_BACKGROUND,
                        VelocirustColor::STATUS_MODAL_VISUAL_FOREGROUND,
                    ),
                    Mode::Terminal => (
                        VelocirustColor::STATUS_MODAL_TERMINAL_BACKGROUND,
                        VelocirustColor::STATUS_MODAL_TERMINAL_FOREGROUND,
                    ),
                };
            ...
            stack((
                svg(move || config.get().ui_svg(VelocirustIcons::SCM)).style(move |s| {
                    let config = config.get();
                ...
                label(branch).style(move |s| {
                    s.margin_left(10.0)
                        .color(config.get().color(VelocirustColor::STATUS_FOREGROUND))
                        .selectable(false)
                }),
            ))
            .style(move |s| {
                ...
                .hover(|s| {
                    s.cursor(CursorStyle::Pointer).background(
                        config.get().color(VelocirustColor::PANEL_HOVERED_BACKGROUND),
                    )
                })
            })
            ...
            .on_event(
                ...
                move |_| {
                    if pointer_down.get() {
                        workbench_command
                            .send(VelocirustWorkbenchCommand::PaletteSCMReferences);
                    }
            ...
            ),
            {
                ...
                stack((
                    svg(move || config.get().ui_svg(VelocirustIcons::ERROR)).style(
                        move |s| {
                            let config = config.get();
                        ...
                        move |s| {
                            s.margin_left(5.0)
                                .color(
                                    config
                                        .get()
                                        .color(VelocirustColor::STATUS_FOREGROUND),
                                )
                                .selectable(false)
                        },
                    ),
                    svg(move || config.get().ui_svg(VelocirustIcons::WARNING)).style(
                        move |s| {
                            let config = config.get();
                            let size = config.ui.icon_size() as f32;
                            s.size(size, size)
        ...
        stack((
            {
                ...
                let icon = {
                    ...
                    move || {
                        ...
                        {
                            VelocirustIcons::SIDEBAR_LEFT
                        } else {
                            VelocirustIcons::SIDEBAR_LEFT_OFF
                        }
                    }
                };
                clickable_icon(
                ...
                let icon = {
                    ...
                    move || {
                        if panel.is_container_shown(
                            ...
                            VelocirustIcons::LAYOUT_PANEL
                        } else {
                            VelocirustIcons::LAYOUT_PANEL_OFF
                        }
                    }
                };
                clickable_icon(
            ...
            },
            {
                ...
                let icon = {
                    ...
                    move || {
                        ...
                        {
                            VelocirustIcons::SIDEBAR_RIGHT
                        } else {
                            VelocirustIcons::SIDEBAR_RIGHT_OFF
                        }
                    }
                };
                clickable_icon(
    ...
    .style(move |s| {
        ...
        s.border_top(1.0)
            ...
            .background(config.color(VelocirustColor::STATUS_BACKGROUND))
...
fn status_text<S: std::fmt::Display + 'static>(
    ...
    label(text).style(move |s| {
        ...
        s.display(display)
            ...
            .padding_horiz(10.0)
            .items_center()
            .color(config.color(VelocirustColor::STATUS_FOREGROUND))
            .hover(|s| {
