...
use velocirust_core::{command::FocusCommand, meta::VERSION, mode::Mode};

use crate::{
    command::{CommandExecuted, CommandKind},
    config::color::VelocirustColor,
...
impl AboutUri {
    const LAPCE: &'static str = "https://velocirust.dev";
    const GITHUB: &'static str = "https://github.com/velocirust/velocirust";
    const MATRIX: &'static str = "https://matrix.to/#/#velocirust-editor:matrix.org";
        ...
        stack((
            svg(move || (config.get()).logo_svg()).style(move |s| {
                s.size(logo_size, logo_size)
                    .color(config.get().color(VelocirustColor::EDITOR_FOREGROUND))
            }),
            label(|| "Velocirust".to_string()).style(move |s| {
                s.font_bold()
                    .margin_top(10.0)
                    .color(config.get().color(VelocirustColor::EDITOR_FOREGROUND))
            }),
            ...
            web_link(
                ...
                move || config.get().color(VelocirustColor::EDITOR_LINK),
                internal_command,
            )
...
fn exclusive_popup<V: View + 'static>(
    ...
    .style(move |s| {
        ...
        .background(
            config
                .get()
                .color(VelocirustColor::LAPCE_DROPDOWN_SHADOW)
                .multiply_alpha(0.5),
        )
    })
}
