...
use crate::{
    ...
    command::{
        CommandKind, InternalCommand, VelocirustCommand, VelocirustWorkbenchCommand,
        WindowCommand,
    },
    config::{
        VelocirustConfig, color::VelocirustColor, icon::VelocirustIcons, ui::TabSeparatorHeight,
    ...
    workspace::{VelocirustWorkspace, VelocirustWorkspaceType},
};
...
#[derive(Parser)]
#[clap(name = "Velocirust")]
#[clap(version=meta::VERSION)]
#[derive(Debug)]
struct Cli {
    /// Launch new window even if Velocirust is already running
    ...
    #[clap(value_parser = velocirust_proxy::cli::parse_file_line_column)]
...
pub struct AppData {
    ...
    pub config: RwSignal<Arc<VelocirustConfig>>,
...
impl AppData {
    pub fn reload_config(&self) {
        let config =
            VelocirustConfig::load(&VelocirustWorkspace::default(), &[], &self.plugin_paths);
    ...
    pub fn new_window(&self, folder: Option<PathBuf>) {
        ...
        let workspace = VelocirustWorkspace {
            path: folder,
            ..Default::default()
        };
    ...
    fn create_windows(
        ...
        if !dirs.is_empty() {
            ...
            for dir in dirs {
                ...
                {
                    VelocirustWorkspaceType::RemoteWSL(crate::workspace::WslHost {
                        host: String::new(),
                    })
                } else {
                    VelocirustWorkspaceType::Local
                };
                #[cfg(not(windows))]
                let workspace_type = VelocirustWorkspaceType::Local;
                ...
                let info = WindowInfo {
                    ...
                    tabs: TabsInfo {
                        active_tab: 0,
                        workspaces: vec![VelocirustWorkspace {
                            kind: workspace_type,
        ...
        if initial_windows == 0 {
            let mut info = db.get_window().unwrap_or_else(|_| WindowInfo {
                ...
                tabs: TabsInfo {
                    active_tab: 0,
                    workspaces: vec![VelocirustWorkspace::default()],
                },
            });
            info.tabs = TabsInfo {
                active_tab: 0,
    ...
    let view_fn = move |(i, layout_rect, child): (
        ...
        let child_view = {
                            ...
                            .apply_if(
                                !info.with(|info| info.is_pristine)
                                    && config.ui.tab_close_button
                                        == TabCloseButton::Off,
                                |s| s.color(config.color(VelocirustColor::LAPCE_WARN)),
                            )
                    })
            })
            .style(|s| s.padding(4.));
            ...
            let tab_close_button = clickable_icon(
                move || {
                    if hovered.get() || info.with(|info| info.is_pristine) {
                        VelocirustIcons::CLOSE
                    } else {
                        VelocirustIcons::UNSAVED
                    }
                },
                move || {
                    let editor_tab_id =
            ...
            .style(move |s| {
                s.items_center()
                    ...
                    .border_right(1.0)
                    .border_color(config.get().color(VelocirustColor::LAPCE_BORDER))
                    .padding_horiz(6.)
                    .gap(6.)
                    .grid()
            ...
            child_view
                ...
                .dragging_style(move |s| {
                    ...
                    s.border(1.0)
                        ...
                        .background(
                            config
                                .color(VelocirustColor::PANEL_BACKGROUND)
                                .multiply_alpha(0.7),
                        )
                        .border_color(config.color(VelocirustColor::LAPCE_BORDER))
                })
                ...
                .style(move |s| {
                    s.size_full()
                        ...
                        .border_color(config.get().color(if is_focused() {
                            VelocirustColor::LAPCE_TAB_ACTIVE_UNDERLINE
                        } else {
                            VelocirustColor::LAPCE_TAB_INACTIVE_UNDERLINE
                        }))
                })
                .style(|s| {
                    s.absolute()
                ...
                .style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .border_color(
                            config
                                .get()
                                .color(VelocirustColor::LAPCE_TAB_ACTIVE_UNDERLINE)
                                .multiply_alpha(0.5),
                        )
                })
        ...
        .style(move |s| {
            ...
            s.height_full()
                ...
                .hover(|s| s.background(config.color(VelocirustColor::HOVER_BACKGROUND)))
        })
    ...
    stack((
            ...
            (
                clip(empty().style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .background(config.color(VelocirustColor::PANEL_BACKGROUND))
                        .box_shadow_blur(3.0)
                        .box_shadow_color(
                            config.color(VelocirustColor::LAPCE_DROPDOWN_SHADOW),
                        )
                }))
                .style(move |s| {
                ...
                stack((
                    clickable_icon(
                        || VelocirustIcons::TAB_PREVIOUS,
                        move || {
                            workbench_command
                                .send(VelocirustWorkbenchCommand::PreviousEditorTab);
                        },
                        || false,
                        || false,
                        || "Previous Tab",
                    ...
                    clickable_icon(
                        || VelocirustIcons::TAB_NEXT,
                        move || {
                            workbench_command
                                .send(VelocirustWorkbenchCommand::NextEditorTab);
                        },
                        || false,
                        || false,
                        || "Next Tab",
        ...
        stack({
            ...
            (
                ...
                stack((
                    clickable_icon(
                        || VelocirustIcons::SPLIT_HORIZONTAL,
                        move || {
                            let editor_tab_id =
                                editor_tab.with_untracked(|t| t.editor_tab_id);
                    ...
                    stack((
                        ...
                        .style(move |s| {
                            s.height_full()
                                ...
                                .border_right(1.0)
                                .border_color(
                                    config.get().color(VelocirustColor::LAPCE_BORDER),
                                )
                        }),
                        container(
                            editor_container_view(
                ...
                .style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .background(
                            config
                                .get()
                                .color(VelocirustColor::EDITOR_DRAG_DROP_BACKGROUND),
                        )
                })
            ...
            .style(move |s| {
                ...
                s.position(Position::Absolute)
                    ...
                    .apply_if(is_dragging, |s| {
                        ...
                        .background(config.get().color(VelocirustColor::EDITOR_CARET))
                    })
                    .hover(|s| {
                        s.cursor(match direction {
                            SplitDirection::Vertical => CursorStyle::ColResize,
    ...
    dyn_stack(
        ...
        move |(_, content)| {
            container(empty().style(move |s| {
                ...
                .background(config.get().color(VelocirustColor::LAPCE_BORDER))
            }))
            .style(move |s| {
...
fn main_split(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        s.border_color(config.color(VelocirustColor::LAPCE_BORDER))
            .background(config.color(VelocirustColor::EDITOR_BACKGROUND))
    ...
    let view = container(
        svg(move || config.get().ui_svg(icon()))
            .style(move |s| {
                ...
                let size = config.ui.icon_size() as f32;
                s.size(size, size)
                    .color(config.color(VelocirustColor::LAPCE_ICON_ACTIVE))
                    .disabled(|s| {
                        s.color(config.color(VelocirustColor::LAPCE_ICON_INACTIVE))
    ...
    .style(move |s| {
        ...
        s.padding(4.0)
            ...
            .apply_if(active_fn(), |s| {
                s.border_color(config.color(VelocirustColor::EDITOR_CARET))
            })
            .hover(|s| {
                s.cursor(CursorStyle::Pointer)
                    .background(config.color(VelocirustColor::PANEL_HOVERED_BACKGROUND))
            })
            .active(|s| {
                s.background(
                    config.color(VelocirustColor::PANEL_HOVERED_ACTIVE_BACKGROUND),
                )
            })
    });
...
fn tooltip_tip<V: View + 'static>(
    ...
    container(child).style(move |s| {
        ...
        s.padding_horiz(10.0)
            ...
            .color(config.color(VelocirustColor::TOOLTIP_FOREGROUND))
            .background(config.color(VelocirustColor::TOOLTIP_BACKGROUND))
            .border(1)
            .border_radius(6)
            .border_color(config.color(VelocirustColor::LAPCE_BORDER))
            .box_shadow_blur(3.0)
            .box_shadow_color(config.color(VelocirustColor::LAPCE_DROPDOWN_SHADOW))
            .margin_left(0.0)
            .margin_top(4.0)
    })
}
...
fn palette_item(
    workspace: Arc<VelocirustWorkspace>,
    i: usize,
    item: PaletteItem,
            ...
            container(
                stack((
                    ...
                    focus_text(
                        ...
                        move || config.get().color(VelocirustColor::EDITOR_FOCUS),
                    )
                    ...
                    .style(move |s| {
                        s.color(config.get().color(VelocirustColor::EDITOR_DIM))
                            .min_width(0.0)
                            .flex_grow(1.0)
                            .flex_basis(0.0)
                    }),
            ...
            container(
                stack((
                    svg(move || {
                        ...
                        config
                            .symbol_svg(&kind)
                            .unwrap_or_else(|| config.ui_svg(VelocirustIcons::FILE))
                    })
                    .style(move |s| {
                        let config = config.get();
                        let size = config.ui.icon_size() as f32;
                        s.min_width(size).size(size, size).margin_right(5.0).color(
                            config.symbol_color(&kind).unwrap_or_else(|| {
                                config.color(VelocirustColor::LAPCE_ICON_ACTIVE)
                            }),
                        )
                    }),
                    focus_text(
            ...
            container(
                stack((
                    svg(move || {
                        let config = config.get();
                        match mode {
                            RunDebugMode::Run => config.ui_svg(VelocirustIcons::START),
                            RunDebugMode::Debug => config.ui_svg(VelocirustIcons::DEBUG),
                        }
                    })
                    .style(move |s| {
                        let config = config.get();
    ...
    .style(move |s| {
        s.width_full()
            ...
            .apply_if(index.get() == i, |style| {
                style.background(
                    config.get().color(VelocirustColor::PALETTE_CURRENT_BACKGROUND),
                )
            })
    })
}
                    ...
                    .style(move |s| {
                        s.width_full().cursor(CursorStyle::Pointer).hover(|s| {
                            s.background(
                                config
                                    .get()
                                    .color(VelocirustColor::PANEL_HOVERED_BACKGROUND),
                            )
                        })
                    })
                },
    ...
    container(
        ...
        .style(move |s| {
            ...
            s.width(config.ui.palette_width() as f64)
                ...
                .border_color(config.color(VelocirustColor::LAPCE_BORDER))
                .flex_col()
                .background(config.color(VelocirustColor::PALETTE_BACKGROUND))
                .pointer_events_auto()
        }),
    )
    .style(move |s| {
            ...
            stack((
                svg(move || {
                    if let MessageType::ERROR = message.typ {
                        config.get().ui_svg(VelocirustIcons::ERROR)
                    } else {
                        config.get().ui_svg(VelocirustIcons::WARNING)
                    }
                })
                .style(move |s| {
                    let config = config.get();
                    ...
                    let color = if let MessageType::ERROR = message.typ {
                        config.color(VelocirustColor::LAPCE_ERROR)
                    } else {
                        config.color(VelocirustColor::LAPCE_WARN)
                    };
                    s.min_width(size)
                        .size(size, size)
                        .margin_right(10.0)
...
fn completion(window_tab_data: Rc<WindowTabData>) -> impl View {
                    ...
                    .style(move |s| {
                        ...
                        s.padding_horiz(5.0)
                            ...
                            .apply_if(active.get() == i, |s| {
                                s.background(
                                    config.color(VelocirustColor::COMPLETION_CURRENT),
                                )
                            })
                            .hover(move |s| {
                                s.background(
    ...
    .style(move |s| {
        ...
        s.position(Position::Absolute)
            ...
            .margin_left(origin.x as f32)
            .margin_top(origin.y as f32)
            .background(config.color(VelocirustColor::COMPLETION_BACKGROUND))
...
fn code_action(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        .margin_left(origin.x as f32)
        .margin_top(origin.y as f32)
        .background(config.get().color(VelocirustColor::COMPLETION_BACKGROUND))
        .border_radius(6.0)
    })
...
fn rename(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        s.position(Position::Absolute)
            ...
            .margin_left(origin.x as f32)
            .margin_top(origin.y as f32)
            .background(config.get().color(VelocirustColor::PANEL_BACKGROUND))
            .border_radius(6.0)
            .padding(6.0)
    })
...
fn window_tab(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        let config = config.get();
        s.size_full()
            .color(config.color(VelocirustColor::EDITOR_FOREGROUND))
            .background(config.color(VelocirustColor::EDITOR_BACKGROUND))
            .font_size(config.ui.font_size() as f32)
            ...
            .class(floem::views::scroll::Handle, |s| {
                s.background(config.color(VelocirustColor::LAPCE_SCROLL_BAR))
            })
    })
...
fn workspace_title(workspace: &VelocirustWorkspace) -> Option<String> {
    let p = workspace.path.as_ref()?;
    let dir = p.file_name().unwrap_or(p.as_os_str()).to_string_lossy();
    Some(match &workspace.kind {
        VelocirustWorkspaceType::Local => format!("{dir}"),
        VelocirustWorkspaceType::RemoteSSH(remote) => format!("{dir} [{remote}]"),
        #[cfg(windows)]
        VelocirustWorkspaceType::RemoteWSL(remote) => format!("{dir} [{remote}]"),
    })
}
        ...
        stack((
            ...
            .dragging_style(move |s| {
                ...
                s.border(1.0)
                    ...
                    .color(
                        config
                            .color(VelocirustColor::EDITOR_FOREGROUND)
                            .multiply_alpha(0.7),
                    )
                    .background(
                        config
            ...
            empty().style(move |s| {
                ...
                s.absolute()
                    ...
                    .height_full()
                    .border_color(
                        config.get().color(VelocirustColor::LAPCE_TAB_ACTIVE_UNDERLINE),
                    )
        ...
        container(clickable_icon(
            ...
            move || {
                window_data.run_window_command(WindowCommand::NewWorkspaceTab {
                    workspace: VelocirustWorkspace::default(),
                    end: true,
                });
            },
            || false,
...
fn window(window_data: WindowData) -> impl View {
    ...
    .window_title(move || {
        ...
        match workspace {
            Some(workspace) => format!("{workspace} - Velocirust"),
            None => "Velocirust".to_string(),
        }
    })
    ...
    .window_menu(move || {
        ...
        if let Some((_, window_tab)) = window_tab {
            ...
            let velocirust_command = window_tab.common.velocirust_command;
            window_menu(velocirust_command, workbench_command)
        } else {
            Menu::new("Velocirust")
        }
    })
...
pub fn launch() {
    ...
    trace!(TraceLevel::INFO, "Starting up Velocirust..");
    ...
    if !cli.wait {
        ...
        if let Err(why) = cmd
            ...
            .stderr(stderr)
            .stdout(stdout)
            .env("LAPCE_LOG", "velocirust_app::app=error,off")
            .spawn()
        {
            eprintln!("Failed to launch velocirust: {why}");
            std::process::exit(1);
        };
        return;
    }
    ...
    // in the existing Velocirust process
    if !cli.new {
        match get_socket() {
            Ok(socket) => {
    ...
    if let Err(err) = velocirust_proxy::register_velocirust_path() {
        tracing::error!("{:?}", err);
    }
    let db = match VelocirustDb::new() {
        Ok(db) => Arc::new(db),
        Err(e) => {
            #[cfg(windows)]
            logging::error_modal("Error", &format!("Failed to create VelocirustDb: {e}"));

            trace!(TraceLevel::ERROR, "Failed to create VelocirustDb: {e}");
            std::process::exit(1);
        }
    };
    let scope = Scope::new();
    ...
    let mut watcher = notify::recommended_watcher(ConfigWatcher::new(tx))
        ...
        .unwrap();
    if let Some(path) = VelocirustConfig::settings_file() {
    ...
    if let Some(path) = VelocirustConfig::keymaps_file() {
        if let Err(err) = watcher.watch(&path, notify::RecursiveMode::Recursive) {
            tracing::error!("{:?}", err);
        }
    }
    ...
    let config = VelocirustConfig::load(&VelocirustWorkspace::default(), &[], &plugin_paths);
    ...
    #[cfg(feature = "updater")]
    {
        ...
        std::thread::Builder::new()
            .name("VelocirustUpdater".to_owned())
            .spawn(move || {
                loop {
...
pub fn try_open_in_existing_process(
    ...
    velocirust_rpc::stdio::write_msg(&mut socket, msg)?;
        ...
        std::thread::spawn(move || -> Result<()> {
            ...
            loop {
                let msg: Option<CoreMessage> =
                    velocirust_rpc::stdio::read_msg(&mut reader)?;
...
pub fn window_menu(
    velocirust_command: Listener<VelocirustCommand>,
    workbench_command: Listener<VelocirustWorkbenchCommand>,
) -> Menu {
    Menu::new("Velocirust")
        .entry({
            let mut menu = Menu::new("Velocirust")
                .entry(MenuItem::new("About Velocirust").action(move || {
                    workbench_command.send(VelocirustWorkbenchCommand::ShowAbout)
                }))
                .separator()
                .entry(
                    Menu::new("Settings...")
                        .entry(MenuItem::new("Open Settings").action(move || {
                            workbench_command
                                .send(VelocirustWorkbenchCommand::OpenSettings);
                        }))
                        .entry(MenuItem::new("Open Keyboard Shortcuts").action(
                            move || {
                                workbench_command.send(
                                    VelocirustWorkbenchCommand::OpenKeyboardShortcuts,
                                );
                            },
                        )),
                )
                .separator()
                .entry(MenuItem::new("Quit Velocirust").action(move || {
                    workbench_command.send(VelocirustWorkbenchCommand::Quit);
                }));
            if cfg!(target_os = "macos") {
                menu = menu
                    .separator()
                    .entry(MenuItem::new("Hide Velocirust"))
        ...
        .entry(
            Menu::new("File")
                .entry(MenuItem::new("New File").action(move || {
                    workbench_command.send(VelocirustWorkbenchCommand::NewFile);
                }))
                .separator()
                .entry(MenuItem::new("Open").action(move || {
                    workbench_command.send(VelocirustWorkbenchCommand::OpenFile);
                }))
                .entry(MenuItem::new("Open Folder").action(move || {
                    workbench_command.send(VelocirustWorkbenchCommand::OpenFolder);
                }))
                .separator()
                .entry(MenuItem::new("Save").action(move || {
                    velocirust_command.send(VelocirustCommand {
                        kind: CommandKind::Focus(FocusCommand::Save),
                        data: None,
                    });
                }))
                .entry(MenuItem::new("Save All").action(move || {
                    workbench_command.send(VelocirustWorkbenchCommand::SaveAll);
                }))
                .separator()
                .entry(MenuItem::new("Close Folder").action(move || {
                    workbench_command.send(VelocirustWorkbenchCommand::CloseFolder);
                }))
                .entry(MenuItem::new("Close Window").action(move || {
                    workbench_command.send(VelocirustWorkbenchCommand::CloseWindow);
                })),
        )
        .entry(
            Menu::new("Edit")
