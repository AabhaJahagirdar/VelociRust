...
use velocirust_core::{command::EditCommand, directory::Directory, mode::Mode};
use velocirust_proxy::plugin::{download_volt, volt_icon, wasi::find_all_volts};
use velocirust_rpc::{
...
use crate::{
    ...
    config::{VelocirustConfig, color::VelocirustColor},
    db::VelocirustDb,
    editor::EditorData,
...
impl PluginData {
    ...
    pub fn volt_installed(&self, volt: &VoltMetadata, icon: &Option<Vec<u8>>) {
        ...
        if !is_latest {
            let url = format!(
                "http://127.0.0.1:8080/api/v1/plugins/{}/{}/latest",
            ...
            std::thread::spawn(move || {
                let info: Option<VoltInfo> = velocirust_proxy::get_url(url, None)
                    .ok()
    ...
    fn load_icon(volt: &VoltInfo) -> Result<VoltIcon> {
        let url = format!(
            "http://127.0.0.1:8080/api/v1/plugins/{}/{}/{}/icon?id={}",
        ...
        let content = match cache_content {
            ...
            None => {
                let resp = velocirust_proxy::get_url(&url, None)?;
    ...
    fn download_readme(
        ...
        let url = format!(
            "http://127.0.0.1:8080/api/v1/plugins/{}/{}/{}/readme",
    ...
    fn query_volts(query: &str, offset: usize) -> Result<VoltsInfo> {
        let url = format!(
            "http://127.0.0.1:8080/api/v1/plugins?q={query}&offset={offset}"
        );
        match reqwest::blocking::get(&url) {
        Ok(response) => {
            // If the server exists, parse the JSON
            Ok(response.json::<VoltsInfo>()?)
        }
        Err(_) => {
            // If the local server isn't running, return an empty set
            // effectively "solving" the UI crash.
            Ok(VoltsInfo {
                plugins: Vec::new(),
                total: 0,
            })
            ...
            label(move || control(local_version_info.clone()))
                .style(move |s| {
                    ...
                    s.margin_left(10)
                        ...
                        .color(
                            config
                                .color(VelocirustColor::LAPCE_BUTTON_PRIMARY_FOREGROUND),
                        )
                        .background(
                            config
                                .color(VelocirustColor::LAPCE_BUTTON_PRIMARY_BACKGROUND),
                        )
                        .hover(|s| {
                            s.cursor(CursorStyle::Pointer).background(
                                config
                                    .color(
                                        VelocirustColor::LAPCE_BUTTON_PRIMARY_BACKGROUND,
                                    )
                                    .multiply_alpha(0.8),
                            )
                        })
                        ...
                        .disabled(|s| {
                            s.background(config.color(VelocirustColor::EDITOR_DIM))
                        })
                        .selectable(false)
                })
                                ...
                                stack((
                                    ...
                                    web_link(
                                        ...
                                        move || {
                                            config
                                                .get()
                                                .color(VelocirustColor::EDITOR_LINK)
                                        },
                                        internal_command,
                                    ),
                                ))
