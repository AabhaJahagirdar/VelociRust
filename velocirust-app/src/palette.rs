...
use velocirust_core::{
    buffer::rope_text::RopeText, command::FocusCommand, language::VelocirustLanguage,
    line_ending::LineEnding, mode::Mode, movement::Movement, selection::Selection,
    syntax::Syntax,
};
use velocirust_rpc::proxy::ProxyResponse;
...
use crate::{
    command::{
        CommandExecuted, CommandKind, InternalCommand, VelocirustCommand, WindowCommand,
    },
    db::VelocirustDb,
    ...
    workspace::{VelocirustWorkspace, VelocirustWorkspaceType, SshHost},
};
...
impl PaletteData {
        ...
        let items = workspaces
            ...
            .filter_map(|w| {
                ...
                let filter_text = match &w.kind {
                    VelocirustWorkspaceType::Local => text,
                    VelocirustWorkspaceType::RemoteSSH(remote) => {
                        format!("[{remote}] {text}")
                    }
                    #[cfg(windows)]
                    VelocirustWorkspaceType::RemoteWSL(remote) => {
                        format!("[{remote}] {text}")
                    }
                };
                Some(PaletteItem {
    ...
    fn get_ssh_hosts(&self) {
        ...
        for workspace in workspaces.iter() {
            if let VelocirustWorkspaceType::RemoteSSH(host) = &workspace.kind {
    ...
    fn get_wsl_hosts(&self) {
        ...
        for workspace in workspaces.iter() {
            if let VelocirustWorkspaceType::RemoteWSL(host) = &workspace.kind {
    ...
    fn set_run_configs(&self, content: String) {
        ...
        if configs.is_none() {
            if let Some(path) = self.workspace.path.as_ref() {
                let path = path.join(".velocirust").join("run.toml");
                self.common
                    .internal_command
    ...
    fn get_run_configs(&self) {
        if let Some(workspace) = self.common.workspace.path.as_deref() {
            let run_toml = workspace.join(".velocirust").join("run.toml");
    ...
    fn get_languages(&self) {
        let langs = VelocirustLanguage::languages();
        let items = langs
            .iter()
        ...
        for (name, profile) in profiles.into_iter() {
            ...
            items.push_back(PaletteItem {
                content: PaletteItemContent::TerminalProfile {
                    name: name.to_owned(),
                    profile: velocirust_rpc::terminal::TerminalProfile {
                        name: name.to_owned(),
        ...
        if let Some(item) = items.get(index) {
            match &item.content {
                PaletteItemContent::PaletteHelp { cmd } => {
                    let cmd = VelocirustCommand {
                        kind: CommandKind::Workbench(cmd.clone()),
                        data: None,
                    };

                    self.common.velocirust_command.send(cmd);
                }
                ...
                PaletteItemContent::Command { cmd } => {
                    self.common.velocirust_command.send(cmd.clone());
                }
                    ...
                    self.common.window_common.window_command.send(
                        WindowCommand::SetWorkspace {
                            workspace: VelocirustWorkspace {
                                kind: VelocirustWorkspaceType::RemoteSSH(host.clone()),
                                path: None,
                                last_open: 0,
                            },
                        },
                ...
                PaletteItemContent::WslHost { host } => {
                    self.common.window_common.window_command.send(
                        WindowCommand::SetWorkspace {
                            workspace: VelocirustWorkspace {
                                kind: VelocirustWorkspaceType::RemoteWSL(host.clone()),
                                path: None,
                                last_open: 0,
                            },
                        },
                ...
                PaletteItemContent::Language { name } => {
                    ...
                    if name.is_empty() || name.to_lowercase().eq("plain text") {
                        ...
                        let lang = match VelocirustLanguage::from_name(name) {
                            Some(v) => v,
                            None => return,
                        };
                ...
                PaletteItemContent::SCMReference { name } => {
                    self.common
                        .velocirust_command
                        .send(crate::command::VelocirustCommand {
                        kind: CommandKind::Workbench(
                            crate::command::VelocirustWorkbenchCommand::CheckoutReference,
                        ),
            ...
            self.common.window_common.window_command.send(
                WindowCommand::SetWorkspace {
                    workspace: VelocirustWorkspace {
                        kind: VelocirustWorkspaceType::RemoteSSH(ssh),
                        path: None,
                        last_open: 0,
                    },
                },
