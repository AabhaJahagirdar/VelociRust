...
use crate::{
    ...
    workspace::{VelocirustWorkspace, WorkspaceInfo},
};
...
pub enum SaveEvent {
    App(AppInfo),
    Workspace(VelocirustWorkspace, WorkspaceInfo),
    RecentWorkspace(VelocirustWorkspace),
    Doc(DocInfo),
    DisabledVolts(Vec<VoltID>),
    WorkspaceDisabledVolts(Arc<VelocirustWorkspace>, Vec<VoltID>),
    PanelOrder(PanelOrder),
}

#[derive(Clone)]
pub struct VelocirustDb {
    folder: PathBuf,
...
impl VelocirustDb {
    ...
    pub fn recent_workspaces(&self) -> Result<Vec<VelocirustWorkspace>> {
        let workspaces =
            std::fs::read_to_string(self.folder.join(RECENT_WORKSPACES))?;
        let workspaces: Vec<VelocirustWorkspace> = serde_json::from_str(&workspaces)?;
        Ok(workspaces)
    }

    pub fn update_recent_workspace(&self, workspace: &VelocirustWorkspace) -> Result<()> {
    ...
    fn insert_recent_workspace(&self, workspace: VelocirustWorkspace) -> Result<()> {
...
fn workspace_folder_name(workspace: &VelocirustWorkspace) -> String {
