...
use crate::{
    ...
    command::{
        CommandExecuted, CommandKind, InternalCommand, VelocirustCommand,
        VelocirustWorkbenchCommand, WindowCommand,
    },
    completion::{CompletionData, CompletionStatus},
    config::VelocirustConfig,
    db::VelocirustDb,
    debug::{DapData, VelocirustBreakpoint, RunDebugMode, RunDebugProcess},
    doc::DocContent,
    ...
    workspace::{VelocirustWorkspace, VelocirustWorkspaceType, WorkspaceInfo},
};
...
pub struct CommonData {
    pub workspace: Arc<VelocirustWorkspace>,
    pub scope: Scope,
    ...
    pub velocirust_command: Listener<VelocirustCommand>,
    pub workbench_command: Listener<VelocirustWorkbenchCommand>,
    ...
    pub config: ReadSignal<Arc<VelocirustConfig>>,
    pub proxy_status: RwSignal<Option<ProxyStatus>>,
    pub mouse_hover_timer: RwSignal<TimerToken>,
    pub breakpoints: RwSignal<BTreeMap<PathBuf, BTreeMap<usize, VelocirustBreakpoint>>>,
...
pub struct WindowTabData {
    ...
    pub proxy: ProxyData,
    pub set_config: WriteSignal<Arc<VelocirustConfig>>,
...
impl WindowTabData {
    ...
    pub fn new(
        ...
        let config = VelocirustConfig::load(
            &workspace,
            &all_disabled_volts,
            &window_common.extra_plugin_paths,
        );
        let velocirust_command = Listener::new_empty(cx);
                    ...
                    .map(|(path, breakpoints)| {
                        (
                            ...
                            breakpoints
                                .into_iter()
                                .map(|b| (b.line, b))
                                .collect::<BTreeMap<usize, VelocirustBreakpoint>>(),
                        )
                    })
                    .collect(),
            );
...

        {
            ...
            window_tab_data.common.velocirust_command.listen(move |cmd| {
                window_tab_data.run_velocirust_command(cmd);
            });
        }
        ...
        if !change_plugins.is_empty() {
            ...
            if config.core.auto_reload_plugin {
                let mut plugin_metas: HashMap<
                    String,
                    velocirust_rpc::plugin::VoltMetadata,
                > = self
                    .plugin
                    .installed
                    .get_untracked()
    ...
    pub fn run_velocirust_command(&self, cmd: VelocirustCommand) {
        match cmd.kind {
    ...
    pub fn run_workbench_command(
        &self,
        cmd: VelocirustWorkbenchCommand,
        data: Option<Value>,
    ) {
        use VelocirustWorkbenchCommand::*;
        match cmd {
            // ==== Modal ====
            EnableModal => {
                    ...
                    open_file(options, move |file| {
                        if let Some(mut file) = file {
                            let workspace = VelocirustWorkspace {
                                kind: VelocirustWorkspaceType::Local,
            ...
            InstallToPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        ...
                        arguments: vec![String::from("-e"), format!(r#"do shell script "ln -sf '{}' /usr/local/bin/velocirust" with administrator privileges"#, std::env::args().next().unwrap())],
                    }
                )
            }
            #[cfg(target_os = "macos")]
            UninstallFromPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        program: String::from("osascript"),
                        arguments: vec![String::from("-e"), String::from(r#"do shell script "rm /usr/local/bin/velocirust" with administrator privileges"#)],
                    }
                )
            }
            JumpLocationForward => {
    ...
    fn handle_core_notification(&self, rpc: &CoreNotification) {
        ...
        match rpc {
            ...
            CoreNotification::Log {
                ...
                use velocirust_rpc::core::LogLevel;
