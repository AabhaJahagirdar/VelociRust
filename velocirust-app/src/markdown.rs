...
use velocirust_core::{language::VelocirustLanguage, syntax::Syntax};
use lapce_xi_rope::Rope;
use lsp_types::MarkedString;
use pulldown_cmark::{CodeBlockKind, CowStr, Event, Options, Parser, Tag};
use smallvec::SmallVec;

use crate::config::{VelocirustConfig, color::VelocirustColor};
                        ...
                        Tag::CodeBlock(kind) => {
                            let language =
                                if let CodeBlockKind::Fenced(language) = kind {
                                    md_language_to_velocirust_language(language)
                                } else {
                                    None
                                };
            ...
            Event::Html(text) => {
                attr_list.add_span(
                    ...
                    default_attrs
                        ...
                        .color(config.color(VelocirustColor::MARKDOWN_BLOCKQUOTE)),
                );
                current_text.push_str(&text);
                pos += text.len();
                builder_dirty = true;
...
fn attribute_for_tag<'a>(
    ...
    match tag {
        ...
        Tag::Link {
            ...
            // TODO: Link support
            Some(default_attrs.color(config.color(VelocirustColor::EDITOR_LINK)))
        }
...
fn md_language_to_velocirust_language(lang: &str) -> Option<VelocirustLanguage> {
    // TODO: There are many other names commonly used that should be supported
    VelocirustLanguage::from_name(lang)
}
...
pub fn highlight_as_code(
    ...
    default_attrs: Attrs,
    language: Option<VelocirustLanguage>,
    text: &str,
    start_offset: usize,
    config: &VelocirustConfig,
) {
