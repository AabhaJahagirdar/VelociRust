...
use crate::{
    ...
    language::{self, VelocirustLanguage},
        ...
        let injection_callback = |language: &InjectionLanguageMarker| {
            let language = match language {
                InjectionLanguageMarker::Name(name) => {
                    VelocirustLanguage::from_name(name)
                }
                InjectionLanguageMarker::Filename(path) => {
                    VelocirustLanguage::from_path_raw(path)
                }
                InjectionLanguageMarker::Shebang(id) => VelocirustLanguage::from_name(id),
            };
            language
...
pub struct Syntax {
    pub rev: u64,
    pub language: VelocirustLanguage,
    pub text: Rope,
...
impl Syntax {
    pub fn init(path: &Path) -> Syntax {
        let language = VelocirustLanguage::from_path(path);
        Syntax::from_language(language)
    }

    pub fn plaintext() -> Syntax {
        Self::from_language(VelocirustLanguage::PlainText)
    }

    pub fn from_language(language: VelocirustLanguage) -> Syntax {
    ...
    pub fn find_enclosing_pair(&self, offset: usize) -> Option<(usize, usize)> {
        if self.language == VelocirustLanguage::Markdown {
